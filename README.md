...
4. The Filter Extracts the Username: When the UsernamePasswordAuthenticationFilter sees the login request, it reads the request body. It looks for a parameter named username and extracts its value. In this case, it gets the String "stud".
5. The Filter Calls Your Service: This is the crucial step. The filter, now holding the string "stud", needs to find the user. It asks Spring for the configured UserDetailsService (which is your CustomUserDetailsService). Then, the filter itself makes the call:
```Java
